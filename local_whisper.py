...
def append_transcription_to_file(transcription, audio_file, output_file):
    ...
    with open(output_file, 'a', encoding='utf-8') as f:
        f.write(f"\n\n--- Transcription of {os.path.basename(audio_file)} ---\n")
...
def initialize_or_append_to_output_file(output_file, verbose=True):
    """
    Check if output file exists and initialize it with header if it doesn't.
    ...
    if not os.path.exists(output_file):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(f"# Daily Transcriptions - {datetime.now().strftime('%Y-%m-%d')}\n")
            f.write(f"# Generated by Local Whisper\n\n")
        return True
    else:
        # File already exists, we'll append to it
        if verbose:
            print(f"Output file {output_file} already exists, will append to it")
            
        # Add a separator for new transcription session
        with open(output_file, 'a', encoding='utf-8') as f:
            f.write(f"\n\n{'=' * 80}\n")
            f.write(f"# New Transcription Session - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"{'=' * 80}\n\n")
        return False
...
def main():
    ...
    # Initialize output file or prepare to append to it
    initialize_or_append_to_output_file(output_file, verbose)
    ...
    print(f"\nAll transcriptions have been appended to {output_file}")
